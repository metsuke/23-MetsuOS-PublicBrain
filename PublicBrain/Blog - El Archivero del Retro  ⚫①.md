---
iaStatus: 8
iaStatus_Model: Raul Carrillo aka Metsuke
iaStatus_Generado: "H"
iaStatus_Supervisado: "-"
iaStatus_Validado: "-"
a11y: 0
checked: 0
lang: ES
translations:
created: 2026-01-28T21:36:50.759Z
modified: 2026-01-28T21:38:14.466Z
supervisado: ""
ACCION: ""
ver_major: 0
ver_minor: 2
ver_rev: 1
nav_primary: []
nav_secondary: []
tags: []
---
# Blog - El Archivero del Retro  ⚫①

![Blog El Archivero del Retro](_resources/a1cf3e2db2af68d01ac275cd53fdf59f_MD5.jpg)

[[Blog]]

Es esta una parte del proyecto que tenia reservada para más adelante, pero la "actualidad" manda, es hora de aprovechar mi fondo en cuanto a cds y disquettes para aportar mi minúsculo grano de arena en este mundillo retro tan activo en nuestros dias.

La idea lleva años en cartera, pero una [conversación reciente en X  🟡③🌐](https://x.com/metsuke/status/2016200580788552104?s=20), en torno a Travelsoft 1.5 t su sistema de protección de numero de licencias, ha terminado con el que os habla ordenando los ultomos 30 años de backup, en busca del código de aquella protección.

El caso es que el lio de ficheros que tengo tras tantos años de zips dentro de zips que me ha sido imposible localizar - a priori - dicha información, esto ha desencadenado que abra uno de los maletones de CDs (y algun diskette, como los de Panda Antivirus), en busca del backup en dvd de la epoca, para ver si tengo las copias ahi.

![Convertido en dibujo pero esa es la maleta real que he revisado hoy](PublicBrain/_resources/6a44fd77b2a98b7278a01ac32390cb7f_MD5.jpg)

Pero este proyecto que estoy bautizando "El archivero del retro" empezo hace años con la recopilación de todos esos cds, algun material en fisico, bastante mas en digital (sobre todo lo publicado en la epoca sobre programación), y la deriva actual con las nuevas revistas  "retro", ya que tengo que ordenar la información, arranquemos el proyecto.

No solo se trata de inventariar CDs, tiene que ver con autores, compañias,juegos, sistemas (hoy encontre mi primera web y su base de datos de maquinitas LCD), libros, revistas, fanzines, tecnicas de desarrollo, código fuente para construir diferentes elementos ... todo lo necesario para, en futuro, tener un agente IA creado desde cero (LLVM incluido) que sea nuestro legado a futuras generaciones, capaz de informar, formar e incluso programar, pero esa, es otra historia.

Por ahora, sirva decir que inicio esta parte de la web, que colgará directamente de [[KB]] y que avanzará en paralelo con el resto del proyecto, incluyendo mis pesquisas en busca de aquel codigo del control de licencias cuyo error indicador de intentos de copia fraudulentas arrojaba un "Error en Reality.sys".

¡Al lio!


![[Plantilla - 1MT#One More Thing]]